...
============================
     cpu.v + rom8x1024_sim.v, 論理シミュレーション用 ROM
           |
           + rom8x1024.v, 動作実験用 ROM
           |
           + ram8x2048_sim.v, 論理シミュレーション用 RAM
           |
           + ram8x2048.v, 動作実験用 RAM
           |
...
//動作実験用
//論理シミュレーション時コメントアウト
// `include "rom8x1024.v"  // ROMの記述（quartus, 動作実験用）
// `include "ram8x2048.v"  // RAMの記述（quartus, 動作実験用）
...
//追加設計 5 ヒント(1)：jpr_sel の入出力用ワイヤの宣言
  ...
  //動作実験用
  //論理シミュレーション時コメントアウト
  // rom8x1024  rom8x1024a(clock_rom, rom_addr, rom_data);
  ...
  //動作実験用
  //論理シミュレーション時コメントアウト
  // ram8x2048  ram8x2048a(clock_ram, ram_addr, ram_write_enable,
  //                           ram_write_data, ram_read_data,
  //                           key_ram_addr, key_ram_wdata, key_ram_wen);
...
//mux32_32_32  jp_sel(XX, XX, XX, XX);
