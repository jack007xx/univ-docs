...
static inline INTERNAL_SIZE_T get_max_fast(void) {
  /* Tell the GCC optimizers that global_max_fast is never larger
     ...
     this.)  */
  if (global_max_fast > MAX_FAST_SIZE) __builtin_unreachable();
...
static void do_check_malloc_state(mstate av) {
  ...
  if (av->top == initial_top(av)) return;
...
static mchunkptr mremap_chunk(mchunkptr p, size_t new_size) {
  ...
  if (total_size == new_size) return p;
...
#define MAYBE_INIT_TCACHE() \
  if (__glibc_unlikely(tcache == NULL)) tcache_init();
#else /* !USE_TCACHE */
#define MAYBE_INIT_TCACHE()
static void tcache_thread_shutdown(void) {
...
libc_hidden_def(__libc_malloc) void __libc_free(void *mem) {
  mstate ar_ptr;
  ...
  if (chunk_is_mmapped(p)) /* release mmapped memory. */
  {
    ...
    if (!mp_.no_dyn_threshold && chunksize_nomask(p) > mp_.mmap_threshold &&
      ...
      LIBC_PROBE(memory_mallopt_free_dyn_thresholds, 2, mp_.mmap_threshold,
                 mp_.trim_threshold);
    }
    munmap_chunk(p);
    return;
...
libc_hidden_def(__libc_free) void *__libc_realloc(void *oldmem, size_t bytes) {
  mstate ar_ptr;
...
libc_hidden_def(__libc_realloc) void *__libc_memalign(size_t alignment,
                                                      size_t bytes) {
...
weak_alias(__libc_memalign, aligned_alloc)
    libc_hidden_def(__libc_memalign) void *__libc_valloc(size_t bytes) {
...
#define REMOVE_FB(fb, victim, pp)                                          \
          ...
          while (tcache->counts[tc_idx] < mp_.tcache_count &&
            ...
            if (SINGLE_THREAD_P)
              *fb = tc_victim->fd;
              ...
              if (__glibc_unlikely(tc_victim == NULL)) break;
            }
      ...
      if (tcache && tc_idx < mp_.tcache_bins) {
        ...
        while (tcache->counts[tc_idx] < mp_.tcache_count &&
          ...
          if (tc_victim != 0) {
            ...
            if (av != &main_arena) set_non_main_arena(tc_victim);
            bin->bk = bck;
            bck->fd = bin;
            tcache_put(tc_victim, tc_idx);
          }
  ...
  size_t tc_idx = csize2tidx(nb);
  if (tcache && tc_idx < mp_.tcache_bins) tcache_nb = nb;
  int return_cached = 0;
...
static void _int_free(mstate av, mchunkptr p, int have_lock) {
  ...
  */
    ...
    if (SINGLE_THREAD_P) have_lock = true;
    ...
    */
    ...
    if (!have_lock) __libc_lock_unlock(av->mutex);
  }
  /*
...
static void *_int_memalign(mstate av, size_t alignment, size_t bytes) {
  ...
  {                                            /*
    ...
    set_head(newp,
             newsize | PREV_INUSE | (av != &main_arena ? NON_MAIN_ARENA : 0));
    set_inuse_bit_at_offset(newp, newsize);
    ...
    if (i == 1 || i >= psindex) {
      ...
      for (mchunkptr p = last(bin); p != bin; p = p->bk) {
        ...
        if (size > psm1 + sizeof(struct malloc_chunk)) {
          ...
          char *paligned_mem =
              (char *)(((uintptr_t)p + sizeof(struct malloc_chunk) + psm1) &
                       ~psm1);
...
static __always_inline int do_set_mallopt_check(int32_t value) { return 1; }
...
int __libc_mallopt(int param_number, int value) {
  ...
  switch (param_number) {
    ...
    case M_ARENA_TEST:
      ...
      break;
    case M_ARENA_MAX:
      if (value > 0) do_set_arena_max(value);
      break;
  }
  __libc_lock_unlock(av->mutex);
  return res;
...
weak_alias(__posix_memalign, posix_memalign) int __malloc_info(int options,
                                                               FILE *fp) {
  /* For now, at least.  */
...
weak_alias(__malloc_info, malloc_info) strong_alias(__libc_calloc, __calloc)
    weak_alias(__libc_calloc, calloc) strong_alias(__libc_free, __free)
        strong_alias(__libc_free, free) strong_alias(__libc_malloc, __malloc)
            strong_alias(__libc_malloc, malloc) strong_alias(__libc_memalign,
                                                             __memalign)
